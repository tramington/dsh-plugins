...
import { fingerprint, createIdleTracker, historyEntry, appendHistoryEntry } from '../lib/engine.js'
...
// ---------- P1.3 变更历史 ----------
console.log('3) historyEntry / appendHistoryEntry')
{
	const entry = historyEntry({ operation: 'edit', goal: { id: 'g-test', revision: 2, objective: '新目标文本' } }, new Date('2026-08-30T12:00:00Z'))
	check('条目含 revision', typeof entry === 'string' && entry.includes('rev 2'))
	check('条目含操作', entry.includes('· edit ·'))
	check('条目含 objective 全文', entry.includes('objective: 新目标文本'))
	check('条目含 ISO 时间', entry.includes('2026-08-30T12:00:00.000Z'))
	check('无效输入 → null', historyEntry(null) === null && historyEntry({ operation: 'edit' }) === null)

	const histRoot = mkdtempSync(join(tmpdir(), 'loop-guard-hist-'))
	try {
		check('追加成功', appendHistoryEntry(join(histRoot, 'h'), 'g-test', entry) === true)
		check('追加成功（二次追加）', appendHistoryEntry(join(histRoot, 'h'), 'g-test', entry) === true)
		const content = readFileSync(join(histRoot, 'h', 'g-test.md'), 'utf8')
		check('文件存在且含两条', content.split('## rev 2').length - 1 === 2)
		check('坏目录 → false 不抛', appendHistoryEntry(null, 'g-test', entry) === false)
	} finally {
		rmSync(histRoot, { recursive: true, force: true })
	}
}
