/**
 * @dsh-local/loop-guard — goal 循环守卫（node 半，事件 + 动态 context）。
 *
 * 功能：
 * - P1.1 空转检测：goal 自动续跑中连续 N 轮"工作区产出指纹无变化" → 运行时
 *   context 注入空转警告，提示暂停或换策略（防 token 空烧）。
 * - P1.3 变更历史：每次 goal 变更（create/edit/pause/resume/complete/block）
 *   自动追加记录到 <工作区>/.goal-history/<goalId>.md（objective 全文 + revision），
 *   供"恶化回滚"读取上一 revision 原文（goals 服务无原生 rollback，回滚 = edit 恢复）。
 *
 * 信号源（独立评估，非自报式）：
 * - goal/changed、session/event(turn/end) 事件 → 轮次边界与变更快照
 * - ctx.goals.get(agent) → 轮次计数（roundsStarted）
 * - agent.session.header.cwd → 工作区路径 → 产出指纹（engine.fingerprint）
 *
 * 崩溃安全设计：
 * 1. 注册全程 try/catch，失败静默降级（不影响任何功能）
 * 2. 指纹扫描失败 → 该 agent 降级为"不检测"（不计数、不警告）
 * 3. 历史写入失败 → 静默降级（不阻塞 goal 循环）
 * 4. 警告经动态 context（函数文本，每步重算）投递：无警告时返回空串，
 *    对 prompt 零污染；context 注册失败静默跳过
 * 5. 卸载本插件即完全恢复原状
 */
import { fingerprint, createIdleTracker, historyEntry, appendHistoryEntry } from './engine.js'
...
/** P1.3 变更历史目录（工作区下）。 */
const HISTORY_DIR = '.goal-history'
...
function apply(ctx) {
	...
	try {
		ctx.effect(() => {
			...
			// goal 变更：清警告（非 active/换 goal）+ P1.3 自动记录变更历史
			disposers.push(ctx.on('goal/changed', ({ agent, change }) => {
				try {
					if (!agent) return
					const key = agent.session.id
					// P1.3 历史记录：change 含每次变更后的 goal 快照（objective 全文）
					if (change && change.goal) {
						const cwd = agent.session.header.cwd
						if (typeof cwd === 'string' && cwd.length > 0) {
							const entry = historyEntry(change)
							if (entry) {
								const ok = appendHistoryEntry(join(cwd, HISTORY_DIR), change.goal.id, entry)
								if (!ok && ctx.logger && typeof ctx.logger.warn === 'function') {
									ctx.logger.warn(`loop-guard: history append failed for goal "${change.goal.id}"`)
								}
							}
						}
					}
